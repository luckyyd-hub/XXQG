...
class ChallengeQuiz(object):
    ...
    def _urlgetan(self,item):
        headers = {
            'Content-Type': 'application/json;charset=UTF-8',
            'User-Agent': "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36"
        }
        url = "http://49.235.90.76:5000/api/questions"
        res = requests.get(url,headers = headers,json = item)
        try:
            answer = json.loads(res.text)["answer"]
            return answer
        except:
            return None
    ...
    def _submit(self):
        ...
        item["options"] = self.options.split(" ")
        bank = self._urlgetan(item)
        #logger.info(f'bank:{bank}')
        #logger.info("item",str(item))
    
        #bank = self.db.query(content=self.content, catagory='挑战题')
        if (bank is not None) and (bank in 'ABCDEF') and (len(bank)>0):
